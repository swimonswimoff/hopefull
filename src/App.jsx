...
function ideaToText(idea) {
  const status = STATUSES.find(s => s.key === idea.status)?.label || "検討中";
  const lines = [`【${idea.title || "無題"}】（${status}）`];
  if (idea.tags?.length) lines.push(`タグ: ${idea.tags.join(", ")}`);
  FIELDS.filter(f => f.key !== "title" && idea[f.key]?.trim()).forEach(f => {
    lines.push(`${f.label}: ${idea[f.key]}`);
  });
  return lines.join("\n");
}
  ...
  return (
    <div style={{ position: "relative" }}>
      <button onClick={() => setOpen(o => !o)} style={{
        ...
        border: `1.5px solid ${current.color}44`, borderRadius: 20,
        padding: "3px 10px 3px 8px", fontSize: 11, fontWeight: 700,
        cursor: "pointer", display: "flex", alignItems: "center", gap: 4,
      }}>
        <span style={{ fontSize: 9 }}>●</span>{current.label}<span style={{ fontSize: 9, opacity: 0.6 }}>▾</span>
      </button>
      {open && (
        <div style={{
          ...
          {STATUSES.map(s => (
            <button key={s.key} onClick={() => { onChange(s.key); setOpen(false); }} style={{
              ...
              color: s.color, padding: "8px 12px",
              fontSize: 12, fontWeight: 700, cursor: "pointer", textAlign: "left",
            }}>
              <span style={{ fontSize: 8 }}>●</span>{s.label}
            </button>
          ))}
        </div>
      )}
  ...
  return (
    <div style={{ marginBottom: 14 }}>
      <div style={{
        ...
        marginBottom: 4, textTransform: "uppercase", transition: "color 0.2s",
      }}>{field.label}</div>
      {field.multiline ? (
        <textarea value={value} onChange={e => onChange(e.target.value)}
          placeholder={field.placeholder} rows={2} style={{
            width: "100%", border: "none",
            borderBottom: `1.5px solid ${isEmpty ? "#F0E6D0" : "#FFCC80"}`,
            fontSize: 13.5, color: "#3D2B0D", background: "transparent",
            resize: "none", lineHeight: 1.8, padding: "2px 0 6px",
          }} />
      ) : (
        <input value={value} onChange={e => onChange(e.target.value)}
          placeholder={field.placeholder} style={{
            width: "100%", border: "none",
            borderBottom: `1.5px solid ${isEmpty ? "#F0E6D0" : "#FFCC80"}`,
            fontSize: 15, fontWeight: 700, color: "#3D2B0D",
...
function CopyButton({ text, label = "コピー", small = false }) {
  ...
  return (
    <button onClick={handleCopy} style={{
      background: copied ? "#E8F5E9" : "#FFF8F0",
      color: copied ? "#2E7D32" : "#C8820A",
      border: `1.5px solid ${copied ? "#A5D6A7" : "#FFCC80"}`,
      borderRadius: 20,
...
function IdeaCard({ idea, onAsk, onStatusChange }) {
  ...
  return (
    <div onMouseEnter={() => setHover(true)} onMouseLeave={() => setHover(false)} style={{
      ...
      boxShadow: hover && !isDone ? "0 6px 24px rgba(230,130,0,0.13)" : "0 2px 8px rgba(0,0,0,0.04)",
      ...
      {idea.title && (
        <div style={{
          ...
          color: isDone ? "#8A8078" : "#3D2B0D", marginBottom: 8,
      ...
      {!isDone && filledFields.filter(f => f.key !== "title").length > 0 && (
        <div>
          {(expanded ? filledFields.filter(f => f.key !== "title") : filledFields.filter(f => f.key !== "title").slice(0, 2)).map(f => (
            <div key={f.key} style={{ marginBottom: 6 }}>
              <span style={{ fontSize: 10, fontWeight: 700, color: "#C8820A", letterSpacing: 1, textTransform: "uppercase", marginRight: 6 }}>{f.label}</span>
              <span style={{ fontSize: 13, color: "#6B5440", lineHeight: 1.7, whiteSpace: "pre-wrap" }}>{idea[f.key]}</span>
            </div>
          ))}
          {filledFields.filter(f => f.key !== "title").length > 2 && (
            <button onClick={() => setExpanded(e => !e)} style={{
              background: "none", border: "none", cursor: "pointer",
              fontSize: 11, color: "#C8820A", fontWeight: 600, padding: "2px 0", marginTop: 2,
            }}>{expanded ? "▲ 閉じる" : `▼ さらに${filledFields.filter(f => f.key !== "title").length - 2}項目`}</button>
          )}
        </div>
      )}

      <div style={{ marginTop: 12, display: "flex", justifyContent: "flex-end", gap: 8 }}>
        <CopyButton text={ideaToText(idea)} label="このアイデアをコピー" small />
        {!isDone && (
          <button onClick={() => onAsk(idea)} style={{
...
function AskModal({ idea, onClose }) {
  ...
  async function sendMessage(userText) {
    ...
    setInput("");
    setLoading(true);
    const fieldSummary = FIELDS.filter(f => idea[f.key]?.trim()).map(f => `${f.label}: ${idea[f.key]}`).join("\n");
    const systemPrompt = `あなたはアイデアの壁打ちパートナーです。以下のアイデアについて相談を受けています：\n\n${fieldSummary || "（内容なし）"}\nタグ: ${idea.tags.join(", ")}\n\n返答は簡潔に、でも具体的に。日本語で答えてください。`;
    try {
      const res = await fetch("https://api.anthropic.com/v1/messages", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ model: "claude-sonnet-4-6", max_tokens: 1000, system: systemPrompt, messages: newMsgs }),
      });
  ...
  return (
    <div style={{
      ...
      <div style={{
        background: "#FFFDF8", borderRadius: 24, width: "100%", maxWidth: 560, maxHeight: "85vh",
        display: "flex", flexDirection: "column", boxShadow: "0 24px 80px rgba(0,0,0,0.18)", overflow: "hidden",
      }}>
        <div style={{ padding: "18px 20px 14px", borderBottom: "1px solid #F0E6D0", background: "linear-gradient(135deg, #FFF8F0, #FFF3E0)" }}>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "16px 20px", display: "flex", flexDirection: "column", gap: 12 }}>
          {!started && (
            <div style={{ textAlign: "center", marginTop: 20 }}>
              ...
              <button onClick={() => { setStarted(true); sendMessage("このアイデアについて壁打ちしてほしいです。まず率直な感想と、次のステップのヒントをください。"); }} style={{
          ...
          {loading && (
            <div style={{ display: "flex", gap: 6, paddingLeft: 4 }}>
              {[0,1,2].map(i => (
                <div key={i} style={{ width: 8, height: 8, borderRadius: "50%", background: "#FF9F43", animation: `bounce 1s ease-in-out ${i*0.15}s infinite` }} />
              ))}
            </div>
          )}
          <div ref={bottomRef} />
        ...
        {started && (
          <div style={{ padding: "12px 16px", borderTop: "1px solid #F0E6D0", display: "flex", gap: 8 }}>
            <input value={input} onChange={e => setInput(e.target.value)}
              ...
              placeholder="さらに質問する..." disabled={loading} style={{
                flex: 1, border: "1.5px solid #F0E6D0", borderRadius: 20,
                padding: "8px 14px", fontSize: 13, background: "#FFF", outline: "none", color: "#3D2B0D",
              }} />
            <button onClick={() => input.trim() && sendMessage(input.trim())} disabled={loading || !input.trim()} style={{
              background: input.trim() ? "linear-gradient(135deg, #FF9F43, #FF6B6B)" : "#EEE",
              color: input.trim() ? "#fff" : "#BBB", border: "none", borderRadius: 20,
              width: 40, height: 40, cursor: input.trim() ? "pointer" : "default", fontSize: 16,
            }}>↑</button>
          </div>
        )}
      </div>
...
export default function Hopeful() {
  const [ideas, setIdeas] = useState(() => {
    try {
      const saved = localStorage.getItem("hopefull-ideas");
  ...
  useEffect(() => {
    localStorage.setItem("hopefull-ideas", JSON.stringify(ideas));
  }, [ideas]);
  ...
  function updateStatus(id, status) { setIdeas(prev => prev.map(i => i.id === id ? { ...i, status } : i)); }

  const hasContent = Object.values(fields).some(v => v.trim());

  function saveIdea() {
    if (!hasContent) return;
    const idea = { id: Date.now(), ...fields, tags: selectedTags.length ? selectedTags : ["メモ"], status: "open", createdAt: new Date().toISOString() };
  ...
  const filterByTag = list => filter === "すべて" ? list : list.filter(i => i.tags.includes(filter));
  const visibleIdeas = filterByTag(statusFilter === "done" ? doneIdeas : activeIdeas);

  const allIdeasText = ideas.map(ideaToText).join("\n\n---\n\n");

  return (
    <div style={{ minHeight: "100vh", background: "linear-gradient(160deg, #FFF8F0 0%, #FFF3E0 60%, #FFEFD5 100%)", fontFamily: "'Hiragino Sans', 'Noto Sans JP', system-ui, sans-serif" }}>
      <style>{`
      ...
      <div style={{ maxWidth: 600, margin: "0 auto", padding: "24px 16px 80px" }}>
        ...
        <div style={{ textAlign: "center", marginBottom: 24, paddingTop: 8 }}>
          <span style={{ fontSize: 28, fontWeight: 900, background: "linear-gradient(135deg, #FF9F43, #FF6B6B)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", letterSpacing: -0.5 }}>ホープフル</span>
        </div>

        {/* Input Card */}
        <div style={{ background: "#FFFFFF", border: "2px solid #F0E6D0", borderRadius: 20, padding: "20px 20px 16px", marginBottom: 20, boxShadow: "0 4px 20px rgba(230,160,60,0.08)" }}>
          {FIELDS.map(f => <FieldInput key={f.key} field={f} value={fields[f.key]} onChange={val => setField(f.key, val)} />)}
            ...
            {TAGS.map(tag => {
              ...
              return (
                <button key={tag} onClick={() => toggleTag(tag)} style={{
                  background: active ? c.bg : "transparent", color: active ? c.text : "#BBAA90",
                  border: `1.5px solid ${active ? c.border : "#E8D8C0"}`, borderRadius: 20,
          ...
          <button onClick={saveIdea} style={{
            width: "100%", background: hasContent ? "linear-gradient(135deg, #FF9F43, #FF6B6B)" : "#F0E6D0",
            color: hasContent ? "#fff" : "#C8B090", border: "none", borderRadius: 14,
            padding: "12px", fontSize: 14, fontWeight: 700, cursor: hasContent ? "pointer" : "default", letterSpacing: 0.5,
          }}>{justSaved ? "✓ 保存しました" : "✦ 書き留める"}</button>
        </div>

        {/* 全アイデアコピー */}
        {ideas.length > 0 && (
          <div style={{ display: "flex", justifyContent: "flex-end", marginBottom: 12 }}>
            <CopyButton text={allIdeasText} label={`全${ideas.length}件をまとめてコピー`} />
          </div>
        )}
        ...
        {ideas.length > 0 && (
          <div style={{ marginBottom: 16 }}>
            <div style={{ display: "flex", gap: 4, marginBottom: 10 }}>
              {[{ key: "active", label: "検討中・進行中", count: activeIdeas.length }, { key: "done", label: "完了", count: doneIdeas.length }].map(t => (
                ...
                }}>{t.label} {t.count > 0 && <span style={{ opacity: 0.6, fontSize: 10 }}>({t.count})</span>}</button>
              ))}
            </div>
            <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
              {["すべて", ...TAGS].map(f => (
                <button key={f} onClick={() => setFilter(f)} style={{
                  background: filter === f ? "#8A7060" : "transparent", color: filter === f ? "#fff" : "#8A7060",
                  border: `1.5px solid ${filter === f ? "#8A7060" : "#E8D8C0"}`, borderRadius: 20,
                  padding: "3px 12px", fontSize: 11, fontWeight: 600, cursor: "pointer",
                }}>{f}</button>
              ))}
            </div>
        ...
        {visibleIdeas.length === 0 && (
          <div style={{ textAlign: "center", padding: "48px 0", color: "#BBAA90" }}>
            ...
            <div style={{ fontSize: 13 }}>{ideas.length === 0 ? "最初のアイデアを書き留めよう" : statusFilter === "done" ? "完了したアイデアはまだありません" : "該当するアイデアがありません"}</div>
          </div>
        )}
      ...
      {askTarget && <AskModal idea={askTarget} onClose={() => setAskTarget(null)} />}
    </div>
  );
}
